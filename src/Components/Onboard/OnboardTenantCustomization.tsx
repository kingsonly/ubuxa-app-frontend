...
// const customDomainRegex = /^https:\/\/www\.[a-zA-Z0-9-]+\.[a-zA-Z]+$/
...
//         if (!customDomainRegex.test(data.domainUrl)) {
//             ctx.addIssue({
//                 code: z.ZodIssueCode.custom,
//                 path: ["domainUrl"],
//                 message: "Custom domain must be in the format https://www.example.com",
//             })
//         }
//     } else {
//         if (!domainRegex.test(data.domainUrl)) {
//             ctx.addIssue({
//                 code: z.ZodIssueCode.custom,
//                 path: ["domainUrl"],
//                 message: "Subdomain must only contain letters, numbers, and hyphens",
//             })
//             console.log("i am bad 2 ", data.domainUrl)
//         }
//     }
// })
...
//                 sameSite: "Lax",
//             })
//             setTenantContext(response.data.tenant)
//             if (response) {
...
//                         {
//                             formErrors.find((e) => e.path[0] === "domainUrl")?.message && (
...
import { useState } from "react"
import { z } from "zod"
import { Palette, Upload, Globe, ArrowRight, Check, X } from "lucide-react"
import { Input } from "@/Components/InputComponent/Input"
import { Button } from "../ui/button"
import { Switch } from "../ui/switch"
import { useApiCall } from "@/utils/useApiCall"
...
const domainRegex = /^[a-z0-9-]+$/i
const customDomainRegex = /^https:\/\/www\.([a-zA-Z0-9-]+\.)+[a-zA-Z]{2,}$/ ///^https:\/\/([a-zA-Z0-9-]+\.)+[a-zA-Z]{2,}$/ ///^https:\/\/www\.[a-zA-Z0-9-]+\.[a-zA-Z]+$/
const customizationSchema = z.object({
    logoUrl: z.string().optional(),
    domainUrl: z.string().min(1, "Domain is required"),
    theme: z.object({
...
const OnboardTenantCustomization = ({ updateTenantStatus }: { updateTenantStatus: (status: string) => void }) => {
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const validation = customizationSchema.safeParse(updatedFormData)
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault()
        setLoading(true)
        const parsed = customizationSchema.safeParse(formData)
        ...
        const isAvailable = await checkDomainAvailability(formData.domainUrl)
        if (!isAvailable) {
            setFormErrors([{
                code: z.ZodIssueCode.custom,
                path: ["domainUrl"],
                message: "Domain is already taken or unavailable",
            }])
        ...
        try {
            const updateData: any = {
                ...formData,
                domainUrl: formData.useCustomDomain ? formData.domainUrl.replace(/^https?:\/\/(www\.)?/, "") : `${formData.domainUrl}.ubuxa.ng`,
            }
    ...
    const checkDomainAvailability = async (domain: string): Promise<boolean> => {
        ...
        try {
            const formattedDomain = formData.useCustomDomain ? domain.replace(/^https?:\/\/(www\.)?/, "") : `${domain}.ubuxa.ng`
            const response = await apiCall({
                endpoint: `/v1/tenants/check-domain/${formattedDomain}`,
                method: "get",
                showToast: false
            })

            setDomainAvailable(response.data.available)
            return response.data.available // Assumes your API returns `{ available: boolean }`
        } catch (error) {
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center space-x-3">
                            <Switch
                                ...
                                onCheckedChange={(checked) => {
                                    const updatedFormData = { ...formData, useCustomDomain: checked }
                                    setFormData(updatedFormData)
                        ...
                        <div className="flex items-center space-x-2">
                            ...
                            {!checkingAvailability && domainAvailable === false && (
                                <span className="text-red-500 text-sm"><X /></span>
                            )}
                            {!checkingAvailability && domainAvailable === true && (
                                <span className="text-green-500 text-sm"><Check /></span>
                            )}
                        </div>
                    </div>
                </div>
