...
export const ProductDetailRow = ({ label, value, showNaira = false, title }: {
  ...
  <div className="flex items-center justify-between gap-2 w-full">
    <Tag name={label} />
    <p className="flex gap-1 items-center justify-end text-xs font-bold text-textDarkGrey w-1/2" title={title} >
...
const ProductSaleDisplay = observer(({
  ...
  const doesParamsExist = Boolean(SaleStore.getParametersByProductId());
  const misc = SaleStore.getMiscellaneousByProductId()?.costs;
  const doesDevicesExist = Boolean(SaleStore.getSelectedDevices());
  // const doesRecipientExist = Boolean(SaleStore.getRecipientByProductId(productId));



  const types: ExtraInfoTypePill[] = ["parameters", "devices", "miscellaneous"];
  ...
  const errors: any = {
    parameters: (getFieldError("parameters")[0]) ?? undefined,
    miscellaneous: (getFieldError("miscellaneousPrices")[0]) ?? undefined,
    devices: (getFieldError("devices")[0]) ?? undefined,
  };
  ...
  return (
    <div className="flex flex-col gap-2 w-full p-2.5 border-[0.6px] border-strokeGreyThree rounded-[20px]">
      ...
      <div className="flex flex-col w-full gap-2 bg-[#F9F9F9] p-3 border-[0.6px] border-strokeGreyThree rounded-[20px]">
        {doesParamsExist && (
          <ExtraInfoSection label="Parameters" onClear={() => SaleStore.removeParameter()} />
        )}
        {misc && Object.keys(misc).length > 0 && (
          <ExtraInfoSection label="Miscellaneous Costs" onClear={() => SaleStore.removeMiscellaneousPrice()} />
        )}
        {doesDevicesExist && (
          <ExtraInfoSection label="Devices" onClear={() => {
            SaleStore.removeDevices();
            SaleStore.removeTentativeDevices(productId);
          }} />
        )}

        <div className="flex items-center justify-between gap-2 w-full">
          <div className="flex flex-wrap items-center w-[100%] gap-1 gap-y-1">
            {types.map(type => (
              <ExtraInfoPill
                key={type}
                type={type}
