...
const TabComponent: React.FC<TabComponentProps> = ({
  ...
  return (
    <div
      className={`flex flex-wrap flex-row items-start sm:items-center justify-between sm:justify-start max-w-full gap-1 bg-white sm:w-max p-1 border-[0.6px] border-strokeGreyThree ${tabsContainerClass ? tabsContainerClass : "rounded-md sm:rounded-full"
        }`}
    >
      {tabs.map((tab) => (
        <div
          key={tab.name}
          className={`flex group items-center justify-center gap-2 px-2 py-1 text-xs font-medium cursor-pointer rounded-full ${activeTab?.name === tab.name
            ? "bg-primaryGradient text-white"
            : "bg-white hover:bg-primaryGradient text-textGrey"
            }`}
