...
import ApiErrorMessage from "../ApiErrorMessage"
import ProceedButton from "../ProceedButtonComponent/ProceedButtonComponent"
import { SingleDeviceForm, SingleDeviceFormSchema } from "./SingleDeviceForm"
import { UploadDeviceForm } from "./UploadDeviceForm"
...
const CreateNewDevice: React.FC<CreatNewDeviceProps> = ({ isOpen, setIsOpen, allDevicesRefresh, formType }) => {
  const { apiCall } = useApiCall()
  const [loading, setLoading] = useState(false)
  const [formData, setFormData] = useState<CombinedFormData>(defaultFormData)
  const [formErrors, setFormErrors] = useState<z.ZodIssue[]>([])
  ...
  const handleFormDataChange = (newFormData: Partial<CombinedFormData>) => {
    const updatedFormData = { ...formData, ...newFormData }
    setFormData(updatedFormData)
    console.log("lol", updatedFormData)
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (formType === "singleUpload") {
        const validatedData = SingleDeviceFormSchema.parse(formData)

        // Filter out empty values and prepare data for API
        const newValidatedData = Object.fromEntries(
          Object.entries(validatedData).filter(([key, value]) => {
            // Always include boolean fields and isTokenable
            if (typeof value === "boolean" || key === "isTokenable") {
              return true
            }
        ...
        const formDataToSend = new FormData()
        formDataToSend.append("file", formData.devicesFile)
        formDataToSend.append("isTokenable", String(formData.isTokenable))
        formDataToSend.append("inventoryId", formData.inventoryId)
        formDataToSend.append("restrictedDigitMode", String(formData.restrictedDigitMode))
      ...
      if (error instanceof z.ZodError) {
        ...
        const message =
          error?.response?.data?.message ||
          `Device${formType === "batchUpload" ? "s" : ""} Creation Failed: Internal Server Error`
        setApiError(message)
      }
    } finally {
      setLoading(false)
  ...
  const isFormFilled = isFormValid//formType === "singleUpload" ? isFormValid : !!formData.devicesFile
  ...
  return (
    <Modal
      ...
      onClose={() => {
        setIsOpen(false)
        DeviceStore.unsetInventory()
      }} layout="right" bodyStyle="pb-[100px]">
      <form className="flex flex-col items-center bg-white" onSubmit={handleSubmit} noValidate>
        ...
        >
          <h2 style={{ textShadow: "1px 1px grey" }} className="text-xl text-textBlack font-semibold font-secondary">
            {formType === "singleUpload" ? "New Device" : "New Devices (Batch Upload)"}
          </h2>
        </div>
