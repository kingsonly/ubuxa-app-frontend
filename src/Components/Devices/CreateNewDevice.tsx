...
const FileFormSchema = z.object({
  devicesFile: z
    .instanceof(File, { message: "Devices file is required" })
    .refine((file) => file === null || ["csv", "xlsx"].includes(file.name.split(".").pop()?.toLowerCase() || ""), {
      message: "Only document files (.csv, .xlsx) are allowed.",
    })
    .nullable()
    .default(null),
...
const CreateNewDevice: React.FC<CreatNewDeviceProps> = ({ isOpen, setIsOpen, allDevicesRefresh, formType }) => {
  ...
  const handleFileChange = (e: {
    ...
    if (files && files.length > 0) {
      ...
      if (!result.success) {
        ...
        setFormErrors((prev) => [...prev.filter((error) => error.path[0] !== name), ...fieldErrors])
      } else {
        setFormErrors((prev) => prev.filter((error) => error.path[0] !== name))
        setFormData((prev) => ({
          ...prev,
  ...
  const isFormFilled = formType === "singleUpload" ? isFormValid : !!formData.devicesFile
